...
app.post('/api/admin/reels', adminAuth, async (req, res) => {
  const { video_id, description } = req.body;
  if (!video_id) return res.status(400).json({ error: 'Video ID is required' });
  try {
    await pool.query(
      'INSERT INTO reels (video_id, username, description, is_approved) VALUES ($1, $2, $3, TRUE)',
      [video_id, 'santgram', description || '']
    );
    res.status(201).json({ message: 'Video added successfully as santgram' });
  } catch (error) {
